...
#	 $i = 0;
	 ...
	 $buf =~ s/%([a-zA-Z0-9_\.-]+);?/$pent{$1}/gse;
	 #$buf =~ s/%([a-zA-Z0-9_\.-]+);?/$i++,print STDERR "$1 $pent{$1},$pent{$1}\n"/gse;
	 #$buf =~ s/%([a-zA-Z0-9_\.-]+);?/$i++,$pent{$1}/gse;
#} while ($i != 0);

my $filename = '/tmp/foo3';
open(my $fh, '+>', $filename) or die "Could not open file '$filename' $!";
print $fh "$buf\n";
close $fh;
...
print "<schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" elementFormDefault=\"drawing\">\n";
    ...
    for (my $i = 0; $i <= $#atts; $i++) {
		  if ($atts[$i] eq '#ATTRGROUPREF'){
				...
				if ($atts[$i] eq "(") {
					...
					print STDERR "$atts[$i]\n";
					 $atts[$i] =~ s/%([a-zA-Z0-9_\.-]+);?/$pent{$1}/gse;
					print STDERR "$atts[$i]\n";
